...
## v1.1.0: 사이트와 서버에서 같은 실험 ID 사용

서버를 v1.1.0 이상으로 업데이트하면 gputl list의 reference가 사이트에서 복사한 ID와 같습니다. 기존 id도 계속 사용할 수 있습니다.

```bash
gputl list
# 사이트에서 복사한 실제 ID로 변경
gputl update --id "server-b/job-..." --completed 250 --total 1000
gputl finish --id "server-b/job-..." --status completed
```

공유 ID를 사용해도 현재 로그인 계정의 실험만 수정할 수 있으며, 다른 서버의 ID는 거절합니다. 개인별 ID를 공개할 필요는 없습니다. 이전 서버는 기존 id를 사용하고, 다음 서버 배포 때 함께 전환하면 됩니다.

진행률이 있으면 대시보드는 **예상 전체 소요시간 = (진행률 기록시각 − 시작시각) × 전체량 ÷ 완료량**으로 계산합니다. 진행률이 바뀔 때 update --completed/--total로 기록하면 시각도 함께 저장됩니다. 직접 지정한 --end가 있으면 우선 사용하고, --clear-end로 해제하면 진행률 추정이 적용됩니다.

이 값은 등록된 진행 단계 기준입니다. GEPA 이후 최종 테스트처럼 카운터에 포함되지 않은 후속 단계의 시간은 별도입니다. 이전 수집기의 진행률에는 기록시각이 없어서 서버 수신시각으로 대략 계산하며 화면에 이를 표시합니다. 0%는 추정하지 않고, 100%여도 실제 종료 결과를 받기 전에는 완료 처리하지 않습니다.

전체 버전·서버 배포 주기는 [버전 관리 안내](RELEASING.md)를 따릅니다. git pull만으로 설치 버전은 바뀌지 않으며 설치 프로그램이 배포한 버전을 status와 서버 상세에서 확인합니다.

## 관리자: 운영·업데이트

대시보드에서 관리자 로그인 → 해당 서버의 **서버 관리**를 열면 현재 설치 버전과 필요한 조치, 그대로 복사할 명령어를 볼 수 있습니다. 웹 변경마다 서버를 업데이트할 필요는 없습니다. [서버 관리 안내](SERVER_MANAGEMENT.md).
