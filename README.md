...
**버전·일괄 서버 배포:** [버전 관리 안내](RELEASING.md) · [변경 이력](CHANGELOG.md). 대시보드는 전체 버전과 각 서버의 설치 버전을 표시합니다. 서버 기능은 원하는 시점에 배포 묶음 단위로 적용하세요.

## 구성

| 위치 | 역할 |
| --- | --- |
| GitHub Pages (`main` 브랜치의 `dist/`) | 읽기 전용 웹 화면. 60초마다 상태 파일 확인 |
